...
const FALLBACK_EXPERIENCE = [
  ...
  },
  {
    ...
    description: "Shipped 10+ typed, reusable UI components, then converted to full-time in 4 months — one of the fastest conversions on the team.",
  },
  {
    role: "Software Developer Intern",
    company: "Hubnex Labs",
    meta: "Gurugram (Remote) · Mar 2024 – Sep 2024",
    description: "Built and maintained React-based client interfaces, lowering defect reports by 30% through structured code reviews.",
  },
  {
    role: "Software Developer Intern",
    company: "Techgicus Software Solutions",
    meta: "Nagpur · Mar 2023 – Dec 2023",
    description: "Independently scoped and launched 5 major features, raising application speed by 25% via code splitting and lazy loading.",
  },
];
...
const Experience = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.batch(".tl-item", {
        start: "top 85%",
        onEnter: (batch) => gsap.to(batch, { opacity: 1, y: 0, duration: 0.9, stagger: 0.1, ease: "power3.out" }),
      });
      ...
      if (lineRef.current) {
        ...
        gsap.to(lineRef.current, {
          strokeDashoffset: 0,
          ease: "none",
          scrollTrigger: { trigger: rootRef.current, start: "top 70%", end: "bottom 80%", scrub: 1 },
        });
      }
    }, rootRef);
    return () => ctx.revert();
  ...
  return (
    <section id="experience" ref={rootRef} className="bg-surface py-[var(--space-section)]">
      <div className="container">
        ...
        <div className="relative pl-8">
          ...
          >
            <line ref={lineRef} x1="1" y1="0" x2="1" y2="1000" stroke="var(--color-primary)" strokeWidth="2" />
          </svg>
          <div className="space-y-10">
            {items.map((item) => (
              <div key={`${item.company}-${item.role}`} className="tl-item relative">
                <span className="absolute -left-[34px] top-1.5 h-3 w-3 rounded-full bg-primary shadow-glow">
                  {item.current && <span className="absolute inset-0 -m-1 animate-ping rounded-full bg-primary opacity-60" />}
                </span>
                <div className="rounded-2xl border border-border bg-surface-raised p-6 shadow-2 transition-all duration-300 hover:-translate-y-1 hover:border-primary/40 hover:shadow-3">
                  <div className="font-heading text-lg font-semibold">{item.role}</div>
